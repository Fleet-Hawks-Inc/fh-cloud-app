describe("Vehicle Reminders tests", () => {
  ...
  afterEach(() => {
    ...
    if (authToken && vehicleRenewal) {
      const serviceUrl = Cypress.env('SERVICE_URL') + '/reminders/record/cypress/delete/' + vehicleRenewal+'/'+serviceReminderType;
      cy.request({
        method: 'DELETE',
        url: serviceUrl,
  ...
  it('should allow users to add Vehicle reminders with all required fields', function () {
    ...
    cy.get('.col-lg-11.pb-3.text-right .btn.btn-success.ng-star-inserted').click();//save btn
    cy.wait(4000);
  ...
  it('should allow users to edit vehicle reminders.', function () {
    ...
    cy.get('.btn-group.flex-wrap.show > div[role="menu"] > a:nth-of-type(2)').click();
    cy.get('input[name="subscribers"]').clear();
    cy.get('input[name="dueDate"]').first().click();
