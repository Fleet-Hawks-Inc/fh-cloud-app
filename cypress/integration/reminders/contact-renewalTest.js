...
describe(" Contact reminders tests", () => {
  ...
  afterEach(() => {
    if (authToken && contactRenewal) {
      const serviceUrl = Cypress.env('SERVICE_URL') + '/contactRenewal/record/cypress/delete/' + contactRenewal;
      cy.request({
        method: 'DELETE',
        url: serviceUrl,
        headers: {
  ...
  it.only('should allow users to edit contact reminders.', function () {
    ...
    cy.get('.ng-star-inserted div[role="menu"] > a:nth-of-type(2)').last().click();//edit
    cy.get('input[name="dueDate"]').first().click();
