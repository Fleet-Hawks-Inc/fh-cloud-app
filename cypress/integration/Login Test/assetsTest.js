...
describe('Assets Test', function () {
    ...
    it('should list the asset which was added and then verify the added asset is listed by name/other properties', function () {
        ...
        cy.get('[class] [class="col-md-2 col-lg-2"]:nth-of-type(1) .form-control').last().type('xyz1456');
        cy.get('.result-suggestions > ul > :nth-child(2)').click({ force: true });//search driver
        cy.get('[class="row px-2"] [role="combobox"]').first().click();
        cy.get('div:nth-of-type(3) > .ng-option-label.ng-star-inserted').last().click();//search asset type
        cy.get('.form-horizontal > .row > :nth-child(3) > .mr-2').first().click();//search
        cy.wait(3000);
        cy.get('.form-horizontal > .row > :nth-child(3) > [type="button"]').first().click();//reset
        cy.get('[class="col-md-6 col-lg-6 text-right page-buttons"] [data-toggle]').first().click();
    ...
    it('should allow user to delete asset', function () {
        ...
        cy.get(':nth-child(8) > .dropdown > .dropdown-menu > [href="javascript:;"]').last().click();
    ...
    it('should give validation error message when required fields are not provided', function () {
        ...
        cy.wait(5000);
        cy.get('[class="col-lg-11 text-right"] #nextBtn').first().click();//save
        cy.get('#form_ > :nth-child(1) > :nth-child(2) > .row > :nth-child(1) > #assetIdentification-error').contains('This Field is not allowed to be empty');
        cy.get('.col-lg-10 > #VIN-error').contains('This Field is not allowed to be empty');
        cy.get('.col-lg-10 > #startDate-error').contains('This Field is not allowed to be empty');
        cy.get('[class=" row pt-3"] [for="assetDetails\.assetType"]').contains('This Field is not allowed to be empty');
        cy.get('[class=" row pt-3"] [for="assetDetails\.year"]').contains('This Field is not allowed to be empty');
        cy.get('[class="row pt-3"] [for="assetDetails\.currentStatus"]').contains('This Field is not allowed to be empty');
        cy.get('[for="assetDetails\.heightUnit"]').contains('This Field is not allowed to be empty');
        cy.get('[class="row pt-3"] [for="assetDetails\.licenceCountryID"]').contains('This Field is not allowed to be empty');
        cy.get('[class=" row pt-3"] [for="assetDetails\.licenceStateID"]').contains('This Field is not allowed to be empty');
        cy.get('[class=" row pt-3"] [for="assetDetails\.licencePlateNumber"]').contains('This Field is not allowed to be empty');
        cy.get('[class=" row pt-3"] [for="assetDetails\.annualSafetyDate"]').contains('This Field is not allowed to be empty');
