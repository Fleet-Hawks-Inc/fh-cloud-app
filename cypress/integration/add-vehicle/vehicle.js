describe('vehicle Test', function () {
  ...
  afterEach(() => {
    if (authToken && vehicleID) {
      const serviceUrl = Cypress.env('SERVICE_URL') + '/vehicles/record/cypress/delete/' + vehicleID
      cy.request({
        method: 'DELETE',
        url: serviceUrl,
  ...
  it('should allow users to add vehicles with all required fields.', function () {
    ...
    cy.get('.col-12 input[name="vehicleIdentification"]').first().type('Vehicle Range Rover');//vehicle number
    ...
    cy.get('#form_ > .m-2 > :nth-child(1) > .col-12 > .bg-white > :nth-child(1) > :nth-child(2) > .row > :nth-child(3) > .form-control').type('A2AZRWDG8KL559834');//vin number
  ...
  it('should list the vehicle which was added and then verify the added vehicle is listed by name/other properties', function () {
    ...
    cy.get('form[method="get"] input[name="vehicleIdentification"').last().type('Vehicle Range Rover');
  ...
  it('should allow user to edit vehicle', function () {
    cy.visit('/#/Login');
    cy.get(':nth-child(1) > .input-group > .form-control').clear();
    cy.get(':nth-child(1) > .input-group > .form-control').type(Cypress.config('testerUserName'));
    cy.get(':nth-child(2) > .input-group > .form-control').clear();
    ...
    cy.get('.dropdown-menu.show > a:nth-of-type(1)').last().click();
    cy.url().then(url => {
      console.log('url', url);
      let newUrl = url.split('/');

      vehicleID = newUrl[newUrl.length - 1];
      cy.setLocalStorage('vehicleID', vehicleID);
    })
    Cypress.on('uncaught:exception', (err, runnable) => {
