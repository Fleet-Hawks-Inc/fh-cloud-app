describe("Add Order", function () {
  ...
  afterEach(() => {
    ...
    if (authToken && orderID) {
      const serviceUrl = Cypress.env('SERVICE_URL') + '/orders/record/cypress/delete/' + orderID
      cy.request({
        method: 'DELETE',
        url: serviceUrl,
  ...
  //   cy.get('[id="collapseShipperArea-0"] [class="row mt-3"]:nth-of-type(2) .ng-invalid').type('Calgary Climbing centre Stronghold, 140');
  //   cy.get('[class="col-lg-11 col-md-11 show-search__result"] [title="Calgary Climbing Centre Stronghold\, 140 15 Ave NW\, Calgary\, AB T2M 0G6\, Canada"]').click();//pickup location
  ...
  it('should give validation error message when required fields are not provided', function () {
    ...
    cy.get('[name="orderData\.customerID"] input').first().click();
    cy.get('div:nth-of-type(1) > .ng-option-label').last().click();//customer id
    cy.get('[name="orderData\.customerID"] [title]').click();
    cy.get('.text-danger > div > div').contains('Customer is required.');
    cy.get('div#collapseReceiverArea-0 > .row  ng-select[role="listbox"] input[role="combobox"]').first().click();
    cy.get('div[role="option"] > .ng-option-label').last().click();//shipper 
    cy.get('[id="collapseReceiverArea-0"] [title="Clear all"]').click();
    cy.get('[id="collapseReceiverArea-0"] .text-danger').contains('Receiver is required.');
    cy.get('[name="freight_type"] input').first().click();
    cy.get('div[role="option"] > .ng-option-label').last().click();//freieght type
    cy.get('[name="freight_type"] [title]').click();
    cy.get('.col-lg-3.col-md-12 > .text-danger > div').contains('Freight fee is required.');
    cy.get('input[name="totalMiles"]').type('500');//miles
  ...
  //   cy.get('[class="col-lg-12 col-md-12 show-search__result"] [title="Calgary Climbing Centre Stronghold\, 140 15 Ave NW\, Calgary\, AB T2M 0G6\, Canada"]').click();//pickup locatio
  //   cy.get('input[name="dropOffLocation"]').type('as');
  //   cy.get('[class="col-lg-12 col-md-12 show-search__result"] [title]').click();//drop location
  //   cy.get('#collapseShipperArea-0 > :nth-child(3) > :nth-child(2) > .input-group > .form-control').first().click();
  //   cy.get('div:nth-of-type(2) > div:nth-of-type(3) > .btn-light').last().click();//pickp date
