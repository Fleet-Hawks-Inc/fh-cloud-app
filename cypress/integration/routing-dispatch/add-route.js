describe("Add Route", function () {
    ...
    afterEach(() => {
        if (authToken && routeID) {
            const serviceUrl = Cypress.env('SERVICE_URL') + '/routes/record/cypress/delete/' + routeID
            cy.request({
                method: 'DELETE',
                url: serviceUrl,
    ...
    it('should allow the user to add-route with required fields', function () {
        ...
        cy.get('[class] tr:nth-of-type(2) [pattern]').type('start journey early in the morning');
        cy.get('[class] tr:nth-of-type(3) [pattern]').type('Drive safe');
    ...
    it('should not allow the user to add-route with optional fields', function () {
        ...
        cy.get('div:nth-of-type(1) > .ng-option-label.ng-star-inserted').last().click();//select asset
        cy.get('[name="driverID"] input').first().click();
        cy.get('.ng-option-label.ng-star-inserted').last().click();//select driver
        Cypress.on('uncaught:exception', (err, runnable) => {
