...
export class CountryStateCity {
    ...
    public static GetAllCountries = (): Array<any> => {
        ...
        cscRep.forEach(country => {
            countries.push({ 'countryName': country.name, 'countryCode': country.isoCode });
     ...
     * @returns countries
     */
    public static GetSpecificCountriesByCode = (countryCode: string[]): Array<any> => {
        let countries: [any];
        countryCode.forEach(countryCode => {
            const cscRep = csc.getCountryByCode(countryCode);
            if (cscRep) {
                countries.push({ 'countryName': cscRep.name, 'countryCode': cscRep.isoCode });
            }
     ...
     * @returns
     */
    public static GetStatesByCountryCode = (countryCodes: string[]): Array<any> => {
        let states: any = [];
   ...
   * @returns
   */
    public static GetCitiesByStateCodes = (countryCode: string, stateCode: string): Array<any> => {
        let cities: [any];
