...
export class ActivityComponent implements OnInit {
  ...
  fetchVehicleName() {
    this.apiService
      .getData(`vehicles/fetch/detail/${this.vehicleId}`)
  ...
  fetchVehicleListing() {
    if (this.lastItemSK !== "end") {
      this.apiService
        .getData(
          `vehicles/fetch/TripData?vehicle=${this.vehicleId}&startDate=${this.start}&endDate=${this.end}&lastKey=${this.lastItemSK}&date=${this.datee}`
        )
        .subscribe((result: any) => {
          ...
          if (result.LastEvaluatedKey !== undefined) {
            this.lastItemSK = encodeURIComponent(
              result.Items[result.Items.length - 1].tripSK
            );
            this.datee = encodeURIComponent(
              result.Items[result.Items.length - 1].dateCreated
            );
          } else {
            this.lastItemSK = "end";
          }
  ...
  fetchFullExport() {
    this.apiService
      .getData(
        `vehicles/fetch/vehicelActivity/list?vehicle=${this.vehicleId}&startDate=${this.start}&endDate=${this.end}`
      )
      .subscribe((result: any) => {
  ...
  generateCSV() {
    if (this.exportData.length > 0) {
      ...
      if (link.download !== undefined) {
        ...
        link.setAttribute(
          "download",
          `${moment().format("YYYY-MM-DD:HH:m")}vehicleActivity-Report.csv`
        );
