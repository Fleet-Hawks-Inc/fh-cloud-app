...
export class BrokerageComponent implements OnInit {
  ...
  dataColumns = [
    ...
    { field: 'pickAddress', header: 'Pickup Location', type: "text" },
    { field: 'dropAddress', header: 'Delivery Location', type: "text" },
                    ...
                    for (let shipData of shipInfo.shippers) {
                        for (let shipPickUp of shipData.pickupPoint) {
                            if (shipPickUp.address.manual) {
                                res.pickAddress = shipPickUp.address.address + ' ' +
                                ...
                                res.pickAddress = shipPickUp.address.pickupLocation
                            }
                            else if (shipPickUp.length > 1) {
                                res.pickAddress = shipPickUp.length - 1
                            }
                        }
                    }
                    for (let receiver of shipInfo.receivers) {
                        for (let receiveDropOf of receiver.dropPoint) {
                            if (receiveDropOf.address.manual) {
                                res.dropAddress = receiveDropOf.address.address + ' ' +
                                ...
                                res.dropAddress = receiveDropOf.address.dropOffLocation
                            }
                            else if (receiveDropOf.length > 1) {
                                res.dropAddress = receiveDropOf.length - 1
                            }
                        }
                    }
                }
