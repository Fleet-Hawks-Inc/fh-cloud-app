...
export class AddJournalComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    let contactList = new Array<any>();
    this.getValidContacts(contactList);
    this.contacts = contactList;
  }

  private getValidContacts(contactList: any[]) {
    let ids = [];
  ...
  calculateTotal() {
    ...
    this.difference = Math.abs(
      this.journal.debitTotalAmount - this.journal.creditTotalAmount
    );
  ...
  fetchJournalByID() {
    this.accountService
      .getData(`journal/${this.journalID}`)
  ...
  deleteDocument(name: string, index: number) {
    this.accountService
      .deleteData(`journal/uploadDelete/${this.journalID}/${name}`)
      .subscribe((result: any) => {
