...
export class VehicleRenewListComponent implements OnInit {
  ...
  fetchTasksList() {
    this.apiService
      .getData("tasks/get/list?type=vehicle")
        ...
        .subscribe((result: any) => {
          ...
          if (result.Items.length > 0) {
            ...
            for (let res of result.Items) {
              ...
              res.vehDetails =
                res.vehStatus.toUpperCase().replace("undefined") +
                "\n" +
                res.vehTasks;
  ...
  sendEmailNotification(value) {
    if (value.status !== undefined && value.status !== "") {
      this.apiService
        .getData(
          `reminders/send/email-notification/${value.reminderID}?type=vehicle&status=${value.status}`
        )
        .subscribe((result) => {
