...
export class AddIncidentComponent implements OnInit {
  ...
  disabledButton() {
    if (
      this.event.driverID == "" ||
      this.event.driverID == null ||
      this.event.vehicleID == "" ||
      this.event.vehicleID == null ||
      this.event.tripID == "" ||
      this.event.tripID == null ||
      this.event.eventDate == "" ||
      this.event.eventDate == null ||
      this.event.eventTime == "" ||
      this.event.eventTime == null ||
      this.event.assigned == "" ||
      this.event.assigned == null ||
      this.event.severity == "" ||
      this.event.severity == null ||
      this.event.incidentType == "" ||
      this.event.incidentType == null ||
      this.event.location.label == "" ||
      this.event.status == "" ||
      this.event.eventSource == "" ||
      this.event.notes.length > 500
    ) {
      return true;
    } else {
      return false;
  ...
  fetchDrivers() {
    this.apiService.getData("drivers/safety").subscribe((result: any) => {
    ...
    if (obj === "uploadedPhotos") {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (filesSize > 10) {
          ...
          this.photoSizeError =
            "Please select file which have size below 10 MB.";
          return;
        } else {
          this.photoSizeError = "";
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (filesSize > 30) {
          ...
          this.videoSizeError =
            "Please select file which have size below 30 MB";
          return;
        } else {
          this.videoSizeError = "";
          let name = files[i].name.split(".");
