...
export class EventListComponent implements OnInit {
  ...
  searchEvents() {
    ...
    this.safetyService
      .getData(
        `critical-events/paging?vehicleID=${this.filter.vehicleID}&date=${this.filter.date}`
      )
  ...
  async fetchEvents(refresh?: boolean) {
    ...
    if (this.lastItemSK !== "end") {
      this.safetyService
        .getData(`critical-events?lastKey=${this.lastItemSK}`)
        .subscribe(async (result: any) => {
          if (result.length === 0) {
            this.dataMessage = Constants.NO_RECORDS_FOUND;
            this.loaded = true;
          ...
          if (result.length > 0) {
            ...
            if (this.events[this.events.length - 1].sk !== undefined) {
              this.lastItemSK = encodeURIComponent(
                this.events[this.events.length - 1].sk
              );
            } else {
              this.lastItemSK = "end";
            }
