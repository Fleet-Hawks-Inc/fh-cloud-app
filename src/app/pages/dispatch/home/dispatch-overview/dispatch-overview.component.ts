...
export class DispatchOverviewComponent implements OnInit {
  ...
  initDataTable() {
    ...
    this.apiService
      .getData("auditLogs/fetch?lastEvaluatedKey=" + this.lastEvaluatedKey)
      .subscribe(
        (result: any) => {
          ...
          result.Items.map((k) => {
            k.eventParams.message = k.eventParams.message.replace(
              "consignor",
              "shipper"
            );
            k.eventParams.message = k.eventParams.message.replace(
          ...
          if (result["LastEvaluatedKey"] !== undefined) {
            const lastEvalKey = result[`LastEvaluatedKey`].logSK.replace(
              /#/g,
              "--"
            );
  ...
  fetchActivitiesCount() {
    this.apiService.getData("auditLogs/get/count").subscribe((result: any) => {
  ...
  goToDetails(eventID: string, type: string) {
    if (type === "driver") {
      ...
      this.router.navigateByUrl(
        `/fleet/maintenance/service-program/detail/${eventID}`
      );
    } else if (type === "order") {
      $("#acttimelinemodal").modal("hide");
      ...
      $("#acttimelinemodal").modal("hide");
      this.router.navigateByUrl(
        `/fleet/maintenance/service-log/detail/${eventID}`
      );
  ...
  fetchAllTrips() {
    ...
    this.apiService.getData("trips").subscribe((result: any) => {
      for (let i = 0; i < result.Items.length; i++) {
        ...
        this.tripGraphData = Object.keys(this.tripsMonths).map(
          (key) => this.tripsMonths[key]
        );
        this.initTripsGraph();
  ...
  fetchAllRoutes() {
    this.spinner.show();
    this.apiService.getData("routes").subscribe((result: any) => {
  ...
  fetchEventActivities() {
    this.spinner.show();
    this.apiService.getData("activities").subscribe(async (result: any) => {
      for (let i = 0; i < result.Items.length; i++) {
        const element = result.Items[i];
  ...
  fetchAceManifest() {
    this.spinner.show();
    this.apiService
      .getData("eManifests/get/ACErecords")
      .subscribe((result: any) => {
        ...
        if (result.Count > 0) {
          ...
          this.aceGraphData = Object.keys(this.aceMonths).map(
            (key) => this.aceMonths[key]
          );
          this.initManifestGraph();
        }
        this.spinner.hide();
  ...
  fetchAciManifest() {
    this.spinner.show();
    this.apiService
      .getData("eManifests/get/ACIrecords")
      .subscribe((result: any) => {
        let data = result.Items;
        ...
        if (result.Count > 0) {
          ...
          this.aciGraphData = Object.keys(this.aciMonths).map(
            (key) => this.aciMonths[key]
          );
          this.initManifestGraph();
        }
        this.spinner.hide();
